...
function initSettings ()
{
    Render = {
        ...
        size       : { x :   2, y :   2 },
        camerapos  : { x :   0, y : 0.5, z :  -4 },
        cameradir  : { x :   0, y :-0.5, z :   4 },
...
function textKey ( i ) {
    ...
    while ( result.length < 2 )
        result += i;
    return '<td><tt>' + result + '</tt></td>';
}
function textCell ( r, g, b )
{
    var key = int2hex( r*256 ) + int2hex( g*256 ) + int2hex( b*256 );
    for ( var i = 0 ; i < allTextCells.length ; i++ )
        if ( allTextCells[i] == key )
            return textKey( i );
    allTextCells.push( key );
    return textKey( allTextCells.length - 1 );
}
function createTable ( data, cellFunction )
{
    var result = '<table border=0 cellpadding=0 cellspacing=0>';
    for ( var i = 0 ; i < data.length ; i++ ) {
        result += '<tr>';
        for ( var j = 0 ; j < data[i].length ; j++ )
            result += cellFunction.apply( null, data[i][j] );
        result += '<tr>';
    }
    result += '</table>';
    return result;
...
function setOutput ( table )
{
    if ( ( typeof( USE_TABLE ) != 'undefined' ) && USE_TABLE ) {
        ...
        //output.innerHTML = createTable( table, textCell );
        output.innerHTML = createTable( table, colorCell );
    } else {
...
function render ()
{
    ...
    var centerdir = unit( Render.cameradir );
    var up = unit( Render.cameraup );
    var right = unit( cross( centerdir, up ) );
    up = unit( cross( right, centerdir ) );
    var fullh = scalar( Render.cameradims.w, right );
    var fullv = scalar( -Render.cameradims.h, up );
